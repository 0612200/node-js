 ...
 **********************************************************************/
 ...
 const getListaDeEstados = function(){
   ...
   //O forEach vai fazer a repetição para pegar todas siglas 
   estadosCidades.estados.forEach(function(dadosUf){
      siglaEstado.push(dadosUf.sigla)

      });
      
      //SiglaEstado é o array onde vamos pegar as informações
      listSiglaJSON.uf = siglaEstado

      // A  QUANTIDADE recebe siglaEstado e faz a contagem de estados (para saber o tamanho)
      listSiglaJSON.quantidade = siglaEstado.length;

      return listSiglaJSON;
   }
   //Confirmara o seu programa
   ...
   const getDadosEstado = function(ufEstado){
      ...
      estadosCidades.estados.forEach(function(dadosEstado){
        if(dadosEstado.sigla == ufEstado){
           listSiglaJSON.uf = dadosEstado.sigla;
           listSiglaJSON.descricao = dadosEstado.nome;
           listSiglaJSON.capital = dadosEstado.capital;
           listSiglaJSON.regiao = dadosEstado.regiao;
        }
        return status;
      })
